...
<body>
    ...
    <div class="container">
        <div id="category_name">
            <h1>Your Search: <?php echo $search;?></h1>
        </div>
        <div class="body">
            <div class="left">
                <?php 
                    ...
                    // }else{
                    // }
                    $sql = "select post.post_img,post.title,post.description,user.first_name,user.last_name,post.post_date,category.category_name,post.post_img,post.post_id from post
                            left join user on post.author = user.user_id
                            left join category on post.category = category.category_id
                            where post.title like '%{$search}%' or user.first_name like '%{$search}%' or category.category_name like '%{$search}%' or user.last_name like '%{$search}%' or post.post_date like '%{$search}%'
                            limit {$offset},{$limit}";
                              ...
                              $sql1 = "select *from post
                                       ...
                                       if(mysqli_num_rows($result1) > 0){
                                               ...
                                               for($i=1;$i<=$total_page;$i++){
                                                ...
                                                echo '<li><a href="search.php?page='.$i.'&search='.$search.'"id='.$active.'>'.$i.'</a></li>';
                                            }
                                            if($page<$total_page){
                                                echo '<li><a href="search.php?page='.($page +1).'&id='.$id.'&search='.$search.'">Next</a></li>';
                                            }
                                            echo "</ul>";
                                       }
                            ?>
